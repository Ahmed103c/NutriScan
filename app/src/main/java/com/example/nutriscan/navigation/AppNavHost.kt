...
fun AppNavHost(
    ...
    NavHost(
        ...
        composable<Route.HomePage> {
            HomePage(
                navigateToHistory = { navController.navigate(Route.HistoryPage) },
                navigateToData = { navController.navigate(Route.DataPage) }
            )
        }

        composable<Route.DataPage>
        {
            DataPage( retour = { navController.popBackStack() } )
        }

        composable<Route.HistoryPage> {
            HistoryPage { navController.popBackStack() }
        }
    }
}
